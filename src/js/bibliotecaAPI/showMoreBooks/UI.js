    ...
    libros.forEach(libro => {
        ...
        libroDiv.innerHTML = `
            <a href="libros/${libro.archivo_url}" class="flex justify-center" target="_blank" style="background-color: #FCFCF7;">
                <img class="p-4 sm:p-8" loading="lazy" src="imagenes/${libro.imagen}" alt="hero img">
            </a>
            <div class="flex flex-col justify-between h-full px-3 sm:px-5 pb-3 sm:pb-5">
                <a href="libros/${libro.archivo_url}" target="_blank">
                    <h5 class="text-lg sm:text-xl font-semibold tracking-tight text-gray-900 dark:text-white">${libro.titulo}</h5>
                </a>
            ...
            <div class="flex items-center justify-end px-3 sm:px-5 pb-3 sm:pb-5">
                <a href="libros/${libro.archivo_url}" target="_blank" class="text-white bg-primary-500 hover:bg-primary-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xs sm:text-lg px-4 py-2 sm:px-5 sm:py-2.5 text-center w-full">Ver Libro</a>
            </div>
        `;
