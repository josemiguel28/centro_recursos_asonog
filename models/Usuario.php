...
*/
    ...
    {
        $query = "SELECT id, estado, confirmado FROM " . self::$tabla;
        return Usuario::SQL($query);
    }
    ...
    public static function getAdminPaginatedUsers(int $limit, int $offset): array {
        $query = "SELECT
                    usuarios.id,
                    usuarios.correo,
                    concat(usuarios.nombre, ' ', usuarios.apellido) nombre,
                    ...
                    usuarios.confirmado
                  FROM usuarios
                  JOIN roles ON roles.id = usuarios.rol
